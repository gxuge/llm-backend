...
from src.app.schemas.exam_agent import Citation
from src.agents.nodes.state import AgentState
from src.agents.nodes.utils import split_dataset_ids
...
async def _ragflow_retrieve(question: str) -> tuple[list[str], list[Citation]]:
    ...
    if isinstance(data_block, dict):
        items = data_block.get("chunks") or data_block.get("documents") or data_block.get("hits") or []
        doc_aggs = data_block.get("doc_aggs") or []
    else:
        items = data.get("chunks") or data.get("documents") or data.get("hits") or []
